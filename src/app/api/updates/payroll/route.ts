  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const poll = async () => {
        ...
        polling = true;
        try {
          const metadata = await getPayrollSyncStatus();
          const signature = JSON.stringify(metadata);
          if (
            metadata.revision > lastRevision ||
            signature !== lastSignature
          ) {
            lastRevision = Math.max(lastRevision, metadata.revision);
          ...
          send(
            `event: error\ndata: ${JSON.stringify({
              error: "Не удалось проверить обновление ФОТ.",
            })}\n\n`,
          );
        } finally {
          polling = false;
      ...
      send(": соединение с обновлениями ФОТ Atlas установлено\n\n");
      void poll();
