  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const poll = async () => {
        ...
        polling = true;
        try {
          const metadata = await getMarketingActualSyncMetadata();
          if (metadata.revision > lastRevision) {
            lastRevision = metadata.revision;
            send(
              `event: marketing-actual\ndata: ${JSON.stringify(metadata)}\n\n`,
            );
          }
        } catch {
          send(
            `event: error\ndata: ${JSON.stringify({
              error: "Не удалось проверить обновление маркетингового факта.",
            })}\n\n`,
          );
        } finally {
          polling = false;
      ...
      send(": соединение с обновлениями Atlas установлено\n\n");
      void poll();
