import { getBitrixSalesSyncStatus } from "@/services/bitrix24-sales-sync";
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const poll = async () => {
        ...
        polling = true;
        try {
          const status = await getBitrixSalesSyncStatus();
          const signature = JSON.stringify(status);
          if (status.revision > lastRevision || signature !== lastSignature) {
            lastRevision = Math.max(lastRevision, status.revision);
            lastSignature = signature;
            send(`event: bitrix24-sales\ndata: ${signature}\n\n`);
          }
        } catch {
          send(
            `event: error\ndata: ${JSON.stringify({
              error: "Не удалось проверить обновление данных Bitrix24.",
            })}\n\n`,
          );
        } finally {
          polling = false;
      ...
      send(": соединение с обновлениями Bitrix24 Atlas установлено\n\n");
      void poll();
